...
pub async fn register(
    ...
    tracing::info!("[register] platform={} pubkey={} token=…{}",
        body.platform,
        body.lair_pubkey,
...
pub async fn unregister(
    ...
    match state.db.delete_subscription(&body.device_token, &body.lair_pubkey) {
        Ok(n) => {
            tracing::info!("[unregister] removed {n} row(s) for pubkey={} token=…{}",
                body.lair_pubkey, tail4(&body.device_token));
            StatusCode::NO_CONTENT.into_response()
        }
        Err(e) => {
            tracing::error!("[unregister] db: {e}");
...
pub async fn notify(
    ...
    let pubkey_b32 = match headers.get("x-lair-pubkey").and_then(|v| v.to_str().ok()) {
        Some(s) => s.to_string(),
        None    => return (StatusCode::BAD_REQUEST, "missing X-Lair-Pubkey").into_response(),
    };
    let sig_b64 = match headers.get("x-lair-sig").and_then(|v| v.to_str().ok()) {
        Some(s) => s,
        None    => return (StatusCode::BAD_REQUEST, "missing X-Lair-Sig").into_response(),
    };

    let pubkey_bytes = match base32::decode(base32::Alphabet::Rfc4648 { padding: false }, &pubkey_b32) {
        Some(v) if v.len() == PUBLIC_KEY_LENGTH => v,
        _ => return (StatusCode::BAD_REQUEST, "X-Lair-Pubkey: bad base32 / wrong length").into_response(),
    };
    let pk_arr: [u8; PUBLIC_KEY_LENGTH] = pubkey_bytes.try_into().unwrap();
    let verifying = match VerifyingKey::from_bytes(&pk_arr) {
        Ok(v) => v,
        Err(_) => return (StatusCode::BAD_REQUEST, "X-Lair-Pubkey: not a valid Ed25519 key").into_response(),
    };

    let sig_bytes = match B64.decode(sig_b64) {
        Ok(v) if v.len() == SIGNATURE_LENGTH => v,
        _ => return (StatusCode::BAD_REQUEST, "X-Lair-Sig: bad base64 / wrong length").into_response(),
    };
    ...
    match state.db.record_nonce(&pubkey_b32, &body.nonce) {
        Ok(true)  => {}
        Ok(false) => return (StatusCode::UNAUTHORIZED, "nonce replay").into_response(),
        Err(e) => {
            tracing::error!("[notify] nonce db: {e}");
            return (StatusCode::INTERNAL_SERVER_ERROR, "db error").into_response();
        }
    }
        ...
        match state.apns.push(&s.device_token, &state.bundle_id, &payload).await {
            ...
            PushOutcome::InvalidToken => {
                ...
                if let Err(e) = state.db.forget_invalid_token(&s.device_token) {
                    tracing::warn!("[notify] failed to forget invalid token: {e}");
                }
            }
            PushOutcome::Failed(msg) => {
                tracing::warn!("[notify] APNs failure for token=…{}: {msg}", tail4(&s.device_token));
            }
        }
    }
    tracing::info!("[notify] pubkey={pubkey_b32} category={} subs={} delivered={} invalid={}",
