...
//! that every agent in the container uses. `octo ssh pubkey` prints that
...
/// from `~/.octo/.ssh/id_ed25519.pub` on the host — the same file the
/// lair container sees at `/data/.ssh/id_ed25519.pub` via the bind mount.
pub async fn pubkey() -> Result<()> {
    let pub_path = octo_core::container_ssh_public_key(&service::config_dir());
    debug!("[ssh] reading pubkey from {}", pub_path.display());
    let text = std::fs::read_to_string(&pub_path)
        .with_context(|| format!(
            "read {} (run `octo init` first, or check that lair has started \
